...
export default function LoginPage() {
  ...
  const handleAdminLogin = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/auth/admin/login', adminData);
      setSession(res.data.user, res.data.access_token);
  ...
  return (
    <div className="min-h-screen app-gradient flex items-center justify-center px-4">
      <Card className="w-full max-w-md p-8 shadow-lg">
        <h1 className="text-3xl font-bold text-center mb-6" data-testid="login-title">
          Welcome Back
        </h1>
        ...
        <div className="text-center mt-6">
          <p className="text-gray-600">
            New here?{' '}
            <button type="button" onClick={() => router.push('/')} className="text-blue-600 hover:underline font-semibold" data-testid="link-create-store">
              Create your website
            </button>
          </p>
        </div>
